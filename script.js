...
async function submitToAirtable(data) {
    try {
        ...
        if (AIRTABLE_CONFIG.baseId && AIRTABLE_CONFIG.baseId !== 'YOUR_BASE_ID') {
            try {
                const response = await fetch(`https://airtable-proxy-oyrb.vercel.app/api/airtable`, {
                    method: 'POST',
                    headers: {
...
async function updateGiftAmountFromAirtable() {
    ...
    try {
        const response = await fetch(`https://airtable-proxy-oyrb.vercel.app/api/airtable?baseId=${AIRTABLE_CONFIG.baseId}&tableName=${encodeURIComponent(AIRTABLE_CONFIG.tableName)}&apiKey=${AIRTABLE_CONFIG.apiKey}`, {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json'
            }
